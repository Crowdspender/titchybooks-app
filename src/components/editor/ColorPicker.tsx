...
const PRESET_COLORS = [
  ...
  '#27C7F9', '#1FB3E0', '#D6F4FE', '#EAF9FE', '#1A1F2E',
  '#0F131C', '#E8E9EC', '#D97706', '#FEF3C7', '#059669',
...
export default function ColorPicker({ value, onChange, className = '', showTransparent = false }: ColorPickerProps) {
            ...
            {showTransparent && (
              <button
                ...
                style={{
                  ...
                  borderColor: value && value.toLowerCase() === 'transparent' ? 'var(--color-primary)' : 'var(--color-border-strong)',
                  boxShadow: value && value.toLowerCase() === 'transparent' ? '0 0 0 2px var(--color-primary-light)' : 'none'
                }}
              >
                {/* Diagonal line to indicate transparent/none */}
                <div className="absolute inset-0" style={{
                  background: 'linear-gradient(to bottom right, transparent calc(50% - 1px), var(--color-text) calc(50% - 1px), var(--color-text) calc(50% + 1px), transparent calc(50% + 1px))'
                }} />
              </button>
            )}
            {PRESET_COLORS.map((color) => (
              <button
                ...
                style={{
                  ...
                  borderColor: value && color && value.toLowerCase() === color.toLowerCase() ? 'var(--color-primary)' : 'transparent',
                  boxShadow: value && color && value.toLowerCase() === color.toLowerCase() ? '0 0 0 2px var(--color-primary-light)' : 'none'
                }}
              />
            ))}
          </div>
